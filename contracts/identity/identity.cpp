...
namespace identity {
    ...
    */
   CONTRACT contract : public identity_base {
   public:
      using identity_base::identity_base;

      void settrust( const name    trustor, ///< the account authorizing the trust
                     const name    trusting, ///< the account receiving the trust
                     const uint8_t trust = 0 )/// 0 to remove, -1 to mark untrusted, 1 to mark trusted
      {
         require_auth( trustor );
         require_recipient( trusting );

         trust_table table( _self, trustor.value );
         auto itr = table.find(trusting.value);
       ...
       * a 32 bit identity prefix based upon hash(name) ^ hash(tapos).
       *
       ...
       */
      void create( const name creator, const uint64_t identity ) {
         require_auth( creator );
         idents_table t( _self, _self.value );
      ...
      void certprop( const name               bill_storage_to, ///< account which is paying for storage
                     const name               certifier,
                     ...
                     const vector<certvalue>& values )
      {
         ...
         for( const auto& value : values ) {
            ...
            if (value.confidence) {
               ...
               auto itr = idx.lower_bound( certrow::key(value.property, trusted, certifier.value) );

               if (itr != idx.end() && itr->property == value.property && itr->trusted == trusted && itr->certifier == certifier) {
                  idx.modify(itr, "0"_n, [&](certrow& row) {
               ...
               auto itr_old = idx.lower_bound( certrow::key(value.property, !trusted, certifier.value) );
               ...
               if (value.property == "owner"_n.value) {
                  eosio_assert(sizeof(name) == value.data.size(), "data size doesn't match name size");
                  name acnt = *reinterpret_cast<const name*>(value.data.data());
                  if (certifier == acnt) { //only self-certitication affects accounts_table
                     accounts_table( _self, acnt.value ).set( identity, acnt );
                  }
               }
            } else {
               bool removed = false;
               ...
               itr = idx.lower_bound( certrow::key(value.property, !trusted, certifier.value) );
               if (itr != idx.end() && itr->property == value.property && itr->trusted == !trusted && itr->certifier == certifier) {
                  idx.erase(itr);
               } else {
                  removed = true;
               ...
               if (value.property == "owner"_n.value) {
                  ...
                  name acnt = *reinterpret_cast<const name*>(value.data.data());
                  if (certifier == acnt) { //only self-certitication affects accounts_table
                     accounts_table( _self, acnt.value ).remove();
                  }
               }
            }
         }
...
EOSIO_DISPATCH( identity::contract, (create)(certprop)(settrust) );
